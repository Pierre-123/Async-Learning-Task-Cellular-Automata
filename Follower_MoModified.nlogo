turtles-own [
  ...
  lane-offset  ;; the side offset from the leader's heading (e.g. -1 for left, +1 for right)
]
...
to attach-turtle  ;; turtle procedure
  ...
  ;; assign lane offset randomly: left or right side
  set lane-offset one-of [-1 1]
...
to turn-turtle  ;; turtle procedure
  ...
  [
    ;; Follow leader but offset to side by lane-offset
    let follow-distance 1.5  ;; distance behind leader
    let side-offset 1 * lane-offset  ;; lateral offset

    ;; Leader's heading in radians
    let leader-heading-radians ([heading] of leader) * (pi / 180)

    ;; Calculate target point relative to leader's position
    let target-x [xcor] of leader - follow-distance * cos leader-heading-radians + side-offset * cos (leader-heading-radians + pi / 2)
    let target-y [ycor] of leader - follow-distance * sin leader-heading-radians + side-offset * sin (leader-heading-radians + pi / 2)

    ;; Face toward that target point instead of exactly leader
